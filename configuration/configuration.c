...
void configuration_parse() {
    ...
    if (!json_is_object(json_configuration)) {
        json_decref(json_root);
        err_msg = string_createf("Cannot parse json file \"%s\". \"configuration\" element is not a json object", conf_file_path);
        error_handle(JSON_DECODE_ERROR, 0, err_msg.s);
    }
    ...
    if (!json_is_object(json_server)) {
        json_decref(json_root);
        err_msg = string_createf("Cannot parse json file \"%s\". \"server\" element is not a json object", conf_file_path);
        error_handle(JSON_DECODE_ERROR, 0, err_msg.s);
    }
    ...
    if (!json_is_string(json_base_url)) {
        json_decref(json_root);
        err_msg = string_createf("Cannot parse json file \"%s\". \"base-url\" element is not a string", conf_file_path);
        error_handle(JSON_DECODE_ERROR, 0, err_msg.s);
    }
